...
function readAndParseTxtFile(filePath) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (textMatch && currentPage) {
            var pageNumber = parseInt(textMatch[1]); // 当前台词在页面上的编号
            var baseX = parseFloat(textMatch[2]); // 初始X坐标
            var baseY = parseFloat(textMatch[3]); // Y坐标
            var group = parseInt(textMatch[4]); // 分组
